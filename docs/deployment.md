...
Public deployment succeeded for commit `6fdf19c` (Cloudflare build `b28d93ee-b913-4856-bad0-4031b9de6272`, active version prefix `6a98e474`). Public URL: https://lanternspell.strahil-peykov.workers.dev/. `/api/health` returned the SQLite authority and `deployment-1` build marker. No account plan was changed and no TURN service was created.

The public endpoint passed the 23-command real WSS protocol scenario (`evidence/deployed-protocol.json`), including simultaneous plans, stale/duplicate confirmations, reconnect before/after commit, and a complete teaching victory with one reward. A separate production UI run hosted, copied the private invitation, joined a second independent browser context, used WASD, and reconnected through the friend menu (`evidence/deployed-ui.json`). All three observed sockets used WSS; no page errors occurred. The runtime courtyard screenshot was inspected with both mages visible. These are public-server tests from one computer, not different-country device tests or performance acceptance.
