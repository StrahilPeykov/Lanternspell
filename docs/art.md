...
The refinement export no longer rewrites `art/wizard.blend` unless `--save-source` is passed. Runtime Sprint is selected above walking speed and crossfades with idle/walk; remote gait derives from received displacement. See traversal.md for source inspection and controls.
