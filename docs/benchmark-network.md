...
Real local two-game-client UI/rejoin passed for mixed full-book and same-tradition seeded-hand chapters (both lesson and guardian). The original real protocol suite passed against new book sessions, followed by actual server process restart and SQLite restoration of committed battle/rewards/seats. Evidence: benchmark-duo.json, benchmark-protocol.json, benchmark-reconstruction.json. Do not run browser/GPU tests concurrently with the lead's performance sampling. Put new temporary captures and logs in ignored `evidence/local/`; curate milestone evidence through the lead. No public deployment or paid service is authorized.
