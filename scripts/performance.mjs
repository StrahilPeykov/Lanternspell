...
try{
 ...
 const high=await sample();await page.screenshot({path:'evidence/performance-1080.png'});
 ...
 const low=await sample();await page.screenshot({path:'evidence/performance-720-low.png'});
 ...
 await writeFile('evidence/performance.json',JSON.stringify(report,null,2));console.log(JSON.stringify({high:high.steadyFrameMs,low:low.steadyFrameMs,renderer:high.inspection.renderer,errors}));
